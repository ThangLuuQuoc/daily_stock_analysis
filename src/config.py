...
class Config:
    ...
    def _parse_market_review_region(cls, value: str) -> str:
        ...
        supported_regions = ('cn', 'hk', 'us', 'jp', 'kr', 'vn', 'both')
        ordered_regions = ('cn', 'hk', 'us', 'jp', 'kr', 'vn')
        ...
        logging.getLogger(__name__).warning(
            f"MARKET_REVIEW_REGION 配置值 '{value}' 无效，已回退为默认值 'cn'（合法值：cn / hk / us / jp / kr / vn / both；支持逗号分隔有效值）"
        )
        return 'cn'
